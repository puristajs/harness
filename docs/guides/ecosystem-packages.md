...
| --- | --- | --- | --- |
| `@purista/harness` | Typed runtime for models, tools, skills, agents, workflows, sessions, state, sandboxes, evaluation, and telemetry. | `npm install @purista/harness`; compose with `defineHarness({ name })` and bind infrastructure with `getInstance(...)`. | [Configuration](./configuration.md), [tools & skills](./tools-and-skills.md), [workflows](./workflows.md) |
| `@purista/harness-ai-sdk-ui` | AI SDK UI Message Stream v1 adapter for `useChat`, AI Elements, and compatible clients. | Parse a UI request with `parseHarnessUIMessageRequest`, then project an agent or workflow stream with `createHarnessUIMessageStreamResponse`. | [Package guide](../../packages/harness-ai-sdk-ui/README.md) |
