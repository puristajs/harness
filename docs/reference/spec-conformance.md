...
- Immutable definitions, sessions, direct agents, workflows, tools, skills, sandbox/state
...
- MCP stdio runs through a spawn-capable sandbox; Streamable HTTP uses an
...
| Definition/runtime API (spec 42) | Aligned | `defineTool`, `defineSkill`, `defineMcpServer`, `defineAgent`, `defineWorkflow`, and `defineCatalog` compose by exact reference; `getInstance` binds projected resources; invokers use `run/stream`; sessions use `release/destroy`. |
| Direct agents | Aligned | `session.agents.<id>.run/stream` is canonical. |
| Workflows | Aligned | Optional orchestration with exact `ctx.agents` maps, agent-call budgets, stable call ids, and child-agent lineage events. |
...
| Living Wiki example | Aligned | Real app shell, standard AI SDK UI Message Stream v1, resumable approval, artifacts, graph, operator view, Jaeger links, and optional draw.io MCP. |
