import { defineHarness, inMemorySandbox, JsonLogger, type GovernanceApprovalProvider, type GovernanceApprovalRequest } from '@purista/harness'
import { FakeModelProvider } from '@purista/harness/testing'
import { createSensitiveDataActions, defineGuardrailAction, defineGuardrails, type SensitiveDataDetector } from '@purista/harness-guardrails'
import { z } from 'zod'
...
export function createGuardrailsExample(options: GuardrailsExampleOptions = {}) {
  ...
  provider.enqueueObject({ object: null, usage, finishReason: 'tool_calls', toolCalls: [
  ...
  const rails = defineGuardrails({
    ...
    actions: {
      ...
      'block unsafe content': defineGuardrailAction({
        ...
        evaluate: ({ value }) => value.includes('[blocked]')
          ? { decision: 'block', reasonCode: 'unsafe_content' }
          : { decision: 'allow' }
      }),
      ...
      'redact note': defineGuardrailAction<'tool_input', typeof publishNoteRailSchema>({
        phase: 'tool_input', tools: ['publish_note'], valueSchema: publishNoteRailSchema,
        evaluate: ({ toolId, value }) => {
          lifecycle.push(`preflight:${toolId}`)
          return value.message.includes('[secret]')
            ? { decision: 'transform', target: 'tool_input', value: { ...value, message: value.message.replaceAll('[secret]', '[redacted]') }, reasonCode: 'secret_redacted' }
            : { decision: 'allow' }
        }
      }),
      'present public status': defineGuardrailAction<'tool_output', typeof publicStatusRailSchema>({
        phase: 'tool_output', tools: ['lookup_status'], valueSchema: publicStatusRailSchema,
        evaluate: () => ({ decision: 'transform' as const, target: 'tool_output' as const, value: { status: 'public status' }, reasonCode: 'private_status_removed' })
      }),
      'redact final answer': defineGuardrailAction({
        phase: 'output',
        valueSchema: z.string(),
        evaluate: ({ value }) => value.includes('[secret]')
          ? { decision: 'transform', target: 'bot_message', value: value.replaceAll('[secret]', '[redacted]'), reasonCode: 'secret_redacted' }
          : { decision: 'allow' }
      })
    }
  })
  const harness = defineHarness({ name: 'guardrails-example' })
    ...
    .agents({ support: rails.attach({
      model: 'assistant', input: z.string(), output: z.string(),
      instructions: ({ input }) => `Answer safely: ${input}`,
      tools: ['lookup_status', 'publish_note'], builtinTools: ['write'],
      permissions: { write: 'require_approval' }
    }) })
    .governance(({ native, rule }) => ({
      defaultEffect: 'allow',
      policies: [native({ id: 'example-review', rules: [
        rule({
          id: 'synthetic-input-audit', tools: ['lookup_status', 'publish_note'], effect: 'audit', reasonCode: 'synthetic_input',
          when: (ctx) => ctx.toolId === 'lookup_status' ? ctx.input.ticket === 'DEMO' : ctx.input.visibility === 'internal'
        }),
        rule({ id: 'review-note', tools: ['publish_note', 'write'], effect: 'require_approval', reasonCode: 'note_review' })
      ] })],
      approval: {
  ...
  return { harness, provider, approvalRequests, handledNotes, lifecycle, get detectorInspections() { return detectorInspections } }
}
...
export async function preflightGuardrailsExample(options: GuardrailsExampleOptions = {}): Promise<GuardrailsExamplePreflight> {
