...
describe('workflow child tasks example', () => {
  it('retrieves a background result and keeps a continuable conversation isolated', async () => {
    ...
    const started = await session.workflows.startReview.run({ documentId: 'DOC-42' })
    ...
    await expect(session.workflows.privateFollowUp.run('first note')).resolves.toMatchObject({ status: 'completed', output: 'follow-up response' })
    await harness.close()
  })
})
