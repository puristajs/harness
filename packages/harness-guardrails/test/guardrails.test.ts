...
import { z } from 'zod'
import { DecisionBlockedError, defineHarness, inMemorySandbox, serializeError, type HarnessAdapterContext } from '@purista/harness'
import { FakeLogger, FakeModelProvider, RecordingTelemetry } from '@purista/harness/testing'
import { createSensitiveDataActions, defineGuardrailAction, defineGuardrails as defineGuardrailsApi, GuardrailsConfigError, modelCheckRail, SensitiveDataDetectorError, sensitiveDataToolRail, type GuardrailActionContext, type SensitiveDataDetector } from '../src/index.js'
...
function defineGuardrails(options: { readonly config: unknown; readonly actions: Record<string, unknown>; readonly observability?: unknown; readonly actionTimeoutMs?: number }) {
  const actions = Object.fromEntries(Object.entries(options.actions).map(([id, action]) => {
    ...
    return [id, definition['phase'] === 'tool_input' || definition['phase'] === 'tool_output'
...
it('normalizes hostile inline configuration errors to one fixed serialized form', () => {
  const hostile = new Proxy({ rails: {} }, {
    get(_target, key) {
      if (key === 'rails') throw new Error('private configuration content')
      return undefined
    }
  })
  expect(() => defineGuardrails({ config: hostile as never, actions: {} })).toThrow(/Guardrails configuration is invalid/)
...
it('normalizes invalid JavaScript error metadata without exposing Zod diagnostics', () => {
  const hostileMeta = new Proxy({}, { get() { throw new Error('private configuration content') } })
  const error = new GuardrailsConfigError(hostileMeta as never)
  expect(serializeError(error)).toEqual({
    code: 'GUARDRAILS_CONFIG_ERROR', category: 'config', retriable: false,
    message: 'Guardrails configuration is invalid.', meta: { reason: 'invalid_shape' }
  })
})

it('recovers from an invalid inline declaration and captures the corrected configuration', async () => {
  expect(() => defineGuardrails({ config: { rails: { retrieval: { flows: ['missing'] } } }, actions: {} })).toThrow(/Guardrails configuration is invalid/)
...
it('rejects raw and forged action objects before evaluation', () => {
  ...
  expect(() => defineGuardrailsApi({ config, actions: { gate: { phase: 'input', evaluate: () => ({ decision: 'allow' }) } } as never })).toThrow(/Guardrails configuration is invalid/)
  expect(() => defineGuardrailsApi({ config, actions: { gate: { phase: 'input' } } as never })).toThrow(/Guardrails configuration is invalid/)
  expect(() => defineGuardrailAction({ phase: 'tool_input', evaluate: () => ({ decision: 'allow' }) } as never)).toThrow(/Invalid guardrail action definition/)
})
...
it('blocks a configured tool-input rail before the Harness tool has a side effect', async () => {
  ...
  await expect(session.agents.answer.prompt('transfer')).rejects.toMatchObject({ code: 'DECISION_BLOCKED' })
  expect(calls).toBe(0)
})
  ...
  const rails = defineGuardrails({
    ...
    actions: {
      'mask transfer memo': sensitiveDataToolRail({
        detector, phase: 'tool_input', tools: ['transfer'], policy: 'input', operation: 'mask',
        valueSchema: z.object({ amount: z.number(), memo: z.string() }),
        codec: {
          id: 'transfer-memo',
          extract: (value) => [{ id: 'memo', text: value.memo }],
          replace: (value, replacements) => ({ ...value, memo: replacements.reduce((memo, replacement) => memo.slice(0, replacement.start) + replacement.value + memo.slice(replacement.end), value.memo) })
        }
      })
    }
  })
...
it('uses a direct Harness model alias for a model-backed check', async () => {
  ...
  const rails = defineGuardrails({
    ...
    actions: { 'self check': modelCheckRail({ phase: 'input', model: 'safety', instructions: 'Return whether the input is allowed.' }) }
  })
  const harness = defineHarness()
    .models({
  ...
  await expect(session.agents.answer.prompt('unsafe question')).rejects.toMatchObject({ code: 'DECISION_BLOCKED' })
  expect(safety.requests).toHaveLength(1)
  expect(assistant.requests).toHaveLength(0)
})

it('derives attached build requirements from only active non-retrieval actions', () => {
  const rails = defineGuardrailsApi({
    ...
    actions: {
      'model check': defineGuardrailAction({ phase: 'input', models: ['safety'], evaluate: () => ({ decision: 'allow' }) }),
      'tool check': defineGuardrailAction({ phase: 'tool_input', tools: ['publish'], evaluate: () => ({ decision: 'allow' }) }),
      'retrieval check': defineGuardrailAction({ phase: 'retrieval', models: ['retrieval_model'], evaluate: () => ({ decision: 'allow' }) })
    }
  })

  const definition = rails.attach({ model: 'assistant', instructions: 'Answer.', tools: ['publish'], builtinTools: false })
  expect(definition.interceptors?.at(-1)).toMatchObject({
...
it('fails attached guardrail preflight before provider work when a declared dependency is unavailable', () => {
  ...
  const rails = defineGuardrailsApi({
    ...
    actions: {
      'model check': defineGuardrailAction({ phase: 'input', models: ['missing'], evaluate: () => ({ decision: 'allow' }) }),
      'tool check': defineGuardrailAction({ phase: 'tool_input', tools: ['publish'], evaluate: () => ({ decision: 'allow' }) })
    }
  })
  ...
  const error = (() => {
    try {
      defineHarness()
        ...
        .tools(({ tool }) => ({ publish: tool({ description: 'Publish.', input: z.object({ message: z.string() }), output: z.boolean(), handler: async () => true }) }))
        .agents({ answer: rails.attach({ model: 'assistant', instructions: 'Answer.', tools: [], builtinTools: false }) })
        .build()
    } catch (value) {
      return value
    }
...
it.each([
  ...
  const rails = defineGuardrailsApi({
    ...
    actions: { 'model check': defineGuardrailAction({ phase: 'input', models: [model], evaluate: () => ({ decision: 'allow' }) }) }
  })
  const models = model === 'missing'
  ...
  const error = (() => {
    try {
      defineHarness().models(models).agents({ answer: rails.attach({ model: 'assistant', instructions: 'Answer.', builtinTools: false }) }).build()
    } catch (value) {
      return value
    }
    throw new Error('Expected attached requirements to fail build validation.')
...
it('projects only declared attached action models and rejects unavailable requirements before action callbacks', async () => {
  const assistant = new FakeModelProvider()
  assistant.enqueue({ object: 'safe answer', usage: { inputTokens: 1, outputTokens: 1, totalTokens: 2 }, finishReason: 'stop' })
  const safety = new FakeModelProvider()
  ...
  try {
    await expect(session.agents.answer.prompt('question')).resolves.toBe('safe answer')
    ...
    const error = (() => {
      try {
        defineHarness()
          ...
          .agents({ answer: rejectedRails.attach({ model: 'assistant', instructions: 'Answer.', builtinTools: false }) })
          .build()
      } catch (value) {
        return value
      }
...
it('checks standalone retrieval model dependencies before any action and projects only declared handles', async () => {
  ...
  await expect(rails.filterRetrievedChunks(['source'])).rejects.toMatchObject({
    code: 'GUARDRAILS_CONFIG_ERROR', meta: { reason: 'model_missing', modelAlias: 'safety' }
  })
  await expect(rails.filterRetrievedChunks(['source'], { models: { safety: {} as never } })).rejects.toMatchObject({
    code: 'GUARDRAILS_CONFIG_ERROR', meta: { reason: 'model_capability_missing', modelAlias: 'safety' }
  })
  expect(actionCalls).toBe(0)

  const model = { object: async () => ({ object: {}, usage: { inputTokens: 0, outputTokens: 0, totalTokens: 0 }, finishReason: 'stop' as const }) }
  await expect(rails.filterRetrievedChunks(['source'], { models: { safety: model, unrelated: model } })).resolves.toEqual(['source'])
  expect(exposedAliases).toEqual(['safety'])
})

it('rejects removed legacy configuration categories', () => {
  expect(() => defineGuardrails({ config: { rails: { dialog: { flows: ['hello'] } } } as never, actions: {} })).toThrow(/Guardrails configuration is invalid/)
})
...
it('makes a block searchable without treating the guardrail evaluation itself as an error', async () => {
  ...
  const rails = defineGuardrails({
    ...
    observability: { telemetry, logger },
    actions: { 'deny restricted source': { phase: 'retrieval', evaluate: () => ({ decision: 'block', reasonCode: 'classification_denied' }) } }
  })

  await expect(rails.filterRetrievedChunks(['restricted source'])).rejects.toMatchObject({
    code: 'DECISION_BLOCKED',
    meta: { evidence: { source: { kind: 'guardrail', id: 'deny restricted source' }, phase: 'retrieval', reasonCode: 'classification_denied' } }
...
it('classifies thrown action failures without leaking the action error content', async () => {
  ...
  const rails = defineGuardrails({
    ...
    observability: { telemetry, logger },
    actions: { 'external classifier': { phase: 'retrieval', evaluate: () => { throw new Error('provider rejected customer-secret@example.test') } } }
  })
...
it('supports model-backed retrieval checks through the typed standalone execution context', async () => {
  ...
  const rails = defineGuardrails({
    ...
    actions: { 'retrieval self check': modelCheckRail({ phase: 'retrieval', model: 'guardrail_model', instructions: 'Return the allow decision.' }) }
  })
...
it('parents model-backed rail usage under the GUARDRAIL span with standard model and token attributes', async () => {
  ...
  const rails = defineGuardrails({
    ...
    actions: { 'safety model': modelCheckRail({ phase: 'retrieval', model: 'safety', instructions: 'Return an allow decision.' }) }
  })
  ...
  const harness = defineHarness({ name: 'guardrails-test' })
    ...
    .workflows({ review: {
      ...
      handler: async (ctx) => {
        const chunks = await rails.filterRetrievedChunks([ctx.input], { models: ctx.models, signal: ctx.signal, logger: ctx.log })
  ...
  try {
    await expect(session.workflows.review.prompt('approved source')).resolves.toBe(1)
...
it('blocks sensitive data and fails closed when a detector returns invalid coordinates', async () => {
  ...
  await expect(rails.filterRetrievedChunks(['address@example.test'])).rejects.toMatchObject({ code: 'DECISION_BLOCKED', meta: { evidence: { reasonCode: 'sensitive_data_detected' } } })
  await expect(rails.filterRetrievedChunks(['invalid'])).rejects.toMatchObject({ code: 'DECISION_EVALUATION_ERROR', meta: { failureKind: 'sensitive_data_invalid_result' } })
})

it.each(['missing_optional_dependency', 'Unsafe private kind', 'x'.repeat(65)])('records only a stable safe detector failure kind: %s', async (kind) => {
  const telemetry = new RecordingTelemetry()
  const logger = new FakeLogger()
  const detector: SensitiveDataDetector = {
    id: 'local-ner',
    ...
    async inspect() { throw new SensitiveDataDetectorError(kind, 'Install package for customer@example.test') }
  }
  const rails = defineGuardrails({
    config: inlineConfig({
      rails: { retrieval: { flows: ['detect sensitive data on retrieval'] } },
...
it('rejects invalid inline sensitive-data configuration and unsupported detector capabilities at construction', () => {
  expect(() => defineGuardrails({
    config: { sensitiveData: { input: { entities: ['EMAIL_ADDRESS'], maskToken: '<MASKED>', scoreThreshold: 0.5, recognizers: [] } } } as never,
    actions: {}
...
it('rejects a phase declaration that does not match its configuration binding', () => {
  ...
  try {
    ...
    expect(error).toMatchObject({ code: 'GUARDRAILS_CONFIG_ERROR', meta: { reason: 'invalid_shape', field: 'rails.input.flows', flowId: 'output action' } })
  }
})
...
it('fails closed on extra outcome fields and schema normalization without recording inspected content', async () => {
  ...
  const invalidResult = defineGuardrails({
    ...
    observability: { telemetry, logger },
    actions: { 'strict result': { phase: 'retrieval', evaluate: () => ({ decision: 'allow', extra: 'customer-secret@example.test' }) } }
  })
  await expect(invalidResult.filterRetrievedChunks(['customer-secret@example.test'])).rejects.toMatchObject({ code: 'DECISION_EVALUATION_ERROR', meta: { failureKind: 'invalid_result' } })
  expect(JSON.stringify({ spans: telemetry.spans, metrics: telemetry.metrics, logs: logger.records })).not.toContain('customer-secret@example.test')
  ...
  const provider = new FakeModelProvider()
  provider.enqueue({ object: {}, toolCalls: [{ id: 'transfer-1', name: 'transfer', arguments: { amount: '10' } }], finishReason: 'tool_calls' })
  const harness = defineHarness()
    .sandbox(inMemorySandbox())
    .models({ assistant: { provider, model: 'fake', capabilities: ['object', 'tool_use'] } })
    .tools(({ tool }) => ({ transfer: tool({ description: 'Transfer.', input: z.object({ amount: z.number() }), output: z.object({ ok: z.boolean() }), handler: async () => ({ ok: true }) }) }))
    .agents({ answer: coercingValueSchema.attach({ model: 'assistant', instructions: 'Answer.', tools: ['transfer'], builtinTools: false }) })
    .build()
  const session = await harness.getSession('coercing-rail')
  await expect(session.agents.answer.prompt('transfer')).rejects.toMatchObject({ code: 'DECISION_EVALUATION_ERROR', meta: { failureKind: 'invalid_result' } })
  await session.release()
  await harness.shutdown()
})
...
it('blocks final output before model-object delivery or assistant persistence', async () => {
  ...
  expect(events).toEqual(expect.arrayContaining([expect.objectContaining({ type: 'run.finished', error: expect.objectContaining({ code: 'DECISION_BLOCKED' }) })]))
...
it('applies output rails only to the final candidate, including stopWhen finalization', async () => {
  const provider = new FakeModelProvider()
  provider.enqueue({ object: 'intermediate tool text', toolCalls: [{ id: 'lookup-1', name: 'lookup', arguments: { id: 'one' } }], finishReason: 'tool_calls' })
  provider.enqueue({ object: 'restricted final', finishReason: 'stop' })
  let railCalls = 0
  let toolCalls = 0
  const rails = defineGuardrails({
    config: inlineConfig({ rails: { output: { flows: ['final gate'] } } }),
    actions: { 'final gate': { phase: 'output', evaluate: () => { railCalls += 1; return { decision: 'block', reasonCode: 'restricted' } } } }
  })
  const harness = defineHarness()
    .sandbox(inMemorySandbox())
    .models({ assistant: { provider, model: 'fake', capabilities: ['object', 'tool_use'] } })
    .tools(({ tool }) => ({ lookup: tool({ description: 'Lookup.', input: z.object({ id: z.string() }), output: z.object({ ok: z.boolean() }), handler: async () => { toolCalls += 1; return { ok: true } } }) }))
    .agents({ answer: rails.attach({ model: 'assistant', instructions: 'Answer.', tools: ['lookup'], builtinTools: false }) })
    .build()
  const session = await harness.getSession('guardrail-tool-final')
  await expect(session.agents.answer.prompt('lookup')).rejects.toMatchObject({ code: 'DECISION_BLOCKED' })
  ...
  stoppedProvider.enqueue({ object: 'restricted stop result', toolCalls: [{ id: 'lookup-2', name: 'lookup', arguments: { id: 'two' } }], finishReason: 'tool_calls' })
  ...
  const stoppedHarness = defineHarness()
    ...
    .tools(({ tool }) => ({ lookup: tool({ description: 'Lookup.', input: z.object({ id: z.string() }), output: z.object({ ok: z.boolean() }), handler: async () => { stoppedToolCalls += 1; return { ok: true } } }) }))
    .agents({ answer: rails.attach({ model: 'assistant', instructions: 'Answer.', tools: ['lookup'], builtinTools: false, stopWhen: ({ toolCalls }) => toolCalls.length > 0 }) })
    .build()
  const stoppedSession = await stoppedHarness.getSession('guardrail-stop-final')
  await expect(stoppedSession.agents.answer.prompt('lookup')).rejects.toMatchObject({ code: 'DECISION_BLOCKED' })
...
it.each([
  { name: 'allow reason', outcome: { decision: 'allow', reasonCode: 'PRIVATE INSPECTED CONTENT' }, failureKind: 'invalid_result' },
  { name: 'block reason', outcome: { decision: 'block', reasonCode: 'PRIVATE INSPECTED CONTENT' }, failureKind: 'invalid_result' },
  { name: 'transform reason', outcome: { decision: 'transform', target: 'relevant_chunks', value: [], reasonCode: 'PRIVATE INSPECTED CONTENT' }, failureKind: 'invalid_result' },
  { name: 'transform extra field', outcome: { decision: 'transform', target: 'relevant_chunks', value: [], extra: 'PRIVATE INSPECTED CONTENT' }, failureKind: 'invalid_transform' }
  ...
  expect(JSON.stringify({ error, spans: telemetry.spans, metrics: telemetry.metrics, logs: logger.records })).not.toContain('PRIVATE INSPECTED CONTENT')
})
...
it('keeps ordered transforms and rejects malformed phase outcomes and transform schema changes', async () => {
  const ordered = defineGuardrails({
    ...
    actions: {
      first: { phase: 'retrieval', evaluate: () => ({ decision: 'transform', target: 'relevant_chunks', value: ['first'] }) },
      second: { phase: 'retrieval', evaluate: ({ value }) => ({ decision: 'transform', target: 'relevant_chunks', value: [...value, 'second'] }) }
    }
  })
  ...
  for (const action of [
    ...
    const rails = defineGuardrails({ config: inlineConfig({ rails: { retrieval: { flows: ['invalid'] } } }), actions: { invalid: action } })
    await expect(rails.filterRetrievedChunks(['safe'])).rejects.toMatchObject({ code: 'DECISION_EVALUATION_ERROR' })
  }

  let parseCount = 0
  const mutatingSchema = z.custom<readonly string[]>((value) => Array.isArray(value) && value.every((item) => typeof item === 'string')).transform((value) => {
    parseCount += 1
  ...
  const mutating = defineGuardrails({
    ...
    actions: { mutating: { phase: 'retrieval', valueSchema: mutatingSchema, evaluate: () => ({ decision: 'transform', target: 'relevant_chunks', value: ['original'] }) } }
  })
  await expect(mutating.filterRetrievedChunks(['original'])).rejects.toMatchObject({ code: 'DECISION_EVALUATION_ERROR', meta: { failureKind: 'invalid_transform' } })

  const zeroEquivalent = defineGuardrails({
    config: inlineConfig({ rails: { retrieval: { flows: ['zero'] } } }),
    actions: { zero: { phase: 'retrieval', valueSchema: z.array(z.number()).transform((values) => values.map(() => 0)), evaluate: () => ({ decision: 'transform', target: 'relevant_chunks', value: [0] }) } }
  })
  await expect(zeroEquivalent.filterRetrievedChunks([-0])).rejects.toMatchObject({ code: 'DECISION_EVALUATION_ERROR', meta: { failureKind: 'invalid_result' } })
})

it('inherits the enclosing tool deadline and fences a late rail continuation', async () => {
  vi.useFakeTimers()
  const provider = new FakeModelProvider()
  provider.enqueue({ object: {}, toolCalls: [{ id: 'lookup-timeout', name: 'lookup', arguments: { id: 'one' } }], finishReason: 'tool_calls' })
  let actionSignal: AbortSignal | undefined
  ...
  const harness = defineHarness()
    ...
    .tools(({ tool }) => ({ lookup: tool({ description: 'Lookup.', input: z.object({ id: z.string() }), output: z.object({ ok: z.boolean() }), handler: async () => { toolCalls += 1; return { ok: true } } }) }))
    .agents({ answer: rails.attach({ model: 'assistant', instructions: 'Answer.', tools: ['lookup'], builtinTools: false,
      interceptors: [{ id: 'clock-skew', beforeTool: () => {
  ...
  try {
    ...
    const result = session.agents.answer.prompt('lookup').catch((error: unknown) => error)
    await actionStarted
...
it.each([false, true])('bounds standalone retrieval with fresh action budgets and explicit deadline=%s', async (explicitDeadline) => {
  vi.useFakeTimers()
  const deadlines: number[] = []
  const started = Date.now()
  const action = {
    phase: 'retrieval' as const,
    evaluate: (context: GuardrailActionContext<'retrieval'>) => new Promise<{ decision: 'allow' }>((resolve) => {
  ...
  try {
    const result = rails.filterRetrievedChunks(['safe'], explicitDeadline ? { deadline: started + 5 } : {})
      .then((value) => ({ value }), (error: unknown) => ({ error }))
    await vi.advanceTimersByTimeAsync(20)
    if (explicitDeadline) {
      expect(await result).toMatchObject({ error: { code: 'DECISION_EVALUATION_ERROR', meta: { failureKind: 'callback_timeout' } } })
...
it.each([
  ['input', 'direct'], ['tool_input', 'direct'], ['input', 'delegated'], ['tool_input', 'delegated']
] as const)('preserves rail-owned %s evidence and %s invocation identity', async (phase, mode) => {
  const provider = new FakeModelProvider()
  const count = mode === 'delegated' ? 2 : 1
  if (phase === 'tool_input') for (let index = 0; index < count; index += 1) {
    provider.enqueueObject({ object: {}, toolCalls: [{ id: 'same-call', name: 'lookup', arguments: { id: 'one' } }], finishReason: 'tool_calls' })
  }
  ...
  const rails = defineGuardrails({
    ...
    actions: {
      ...
      'block second': { phase, evaluate: (context) => { contexts.push(context); return { decision: 'block', reasonCode: 'restricted' } } }
    }
  })
  const harness = defineHarness().sandbox(inMemorySandbox()).models({ fake: { provider, model: 'fake', capabilities: ['object', 'tool_use'] } })
    .tools(({ tool }) => ({ lookup: tool({ description: 'Lookup.', input: z.object({ id: z.string() }), output: z.boolean(), handler: async () => { handlers += 1; return true } }) }))
    .agents({ answer: rails.attach({ model: 'fake', instructions: 'Answer.', input: z.string(), output: z.string(), builtinTools: false, tools: ['lookup'],
      interceptors: [{ id: 'occurrence', beforeInput: ({ invocationId, runId }) => { occurrences.push({ invocationId, runId }); return { decision: 'allow' } } }]
    }) })
    .workflows({ review: { input: z.string(), output: z.string(), delegation: { agents: ['answer'] }, handler: async (ctx) => {
      for (let index = 0; index < count; index += 1) {
        try { await ctx.agents.answer(ctx.input) } catch (error) {
  ...
  if (mode === 'delegated') {
    ...
    try { for await (const event of session.agents.answer.stream('question')) events.push(event) } catch (error) {
      if (!(error instanceof DecisionBlockedError)) throw error
      failures.push(error)
    }
  }
  ...
  for (const [index, context] of contexts.entries()) {
    ...
    if (phase === 'tool_input') expect(invocationId).toBe(mode === 'delegated' ? started[index]?.delegationCallId : started[index]?.runId)
    const decisionId = `decision_${createHash('sha256').update(JSON.stringify([
      runId, invocationId, phase, 0, phase === 'tool_input' ? 'lookup' : null, phase === 'tool_input' ? 'same-call' : null,
      'guardrail', 'block second', null, 'block second', 1
    ])).digest('hex')}`
    expect(failures[index]?.meta).toEqual({ evidence: { decisionId, source: { kind: 'guardrail', id: 'block second', ruleId: 'block second' }, phase, reasonCode: 'restricted' } })
  }
  if (mode === 'delegated') expect(failures[0]?.meta?.evidence.decisionId).not.toBe(failures[1]?.meta?.evidence.decisionId)
