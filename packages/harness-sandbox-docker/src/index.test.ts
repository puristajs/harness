...
import { HarnessConfigError, InMemoryHarnessStorage, OperationCancelledError, OperationTimeoutError, SandboxError, SandboxStateLostError, ValidationError, defineHarness, inMemoryHarnessStorage, type HarnessAdapterContext, type SandboxScope } from '@purista/harness'
import { RecordingTelemetry, sandboxContract } from '@purista/harness/testing'
import { z } from 'zod'
...
async function open(adapter: DockerSandbox, chosen: SandboxScope = scope) {
  ...
  return await adapter.open({ scope: chosen, mode: 'create', ...(chosen.owner.identity ? { identity: chosen.owner.identity } : {}) })
}
...
function noLiveSessionHarness(adapter: DockerSandbox, storage = inMemoryHarnessStorage()) {
  return defineHarness()
    ...
    .skills({})
    .agents({
      noop: { model: 'fake', input: z.string(), output: z.string(), instructions: 'Return the provided input.', handler: async (ctx) => ctx.input }
    })
    .build()
}
afterEach(async () => {
  for (const adapter of clients.splice(0)) for (const chosen of scopes.get(adapter) ?? []) await adapter.terminate({ scope: chosen, reason: 'manual' }).catch(() => undefined)
  scopes.clear()
...
describe('Docker sandbox public configuration', () => {
  it('supports no-live get, release, and close across a restart without Docker allocation', async () => {
    ...
    await expect(storage.getSession('docker-no-live-owner')).resolves.toEqual(expect.objectContaining({
  ...
  it('validates closed options before invoking any CLI', () => {
    const invalid = [
      { root: 'relative', image }, { root: '/private/data', image: 'image:latest' },
      { root: '/private/data', image, user: '0:0' }, { root: '/private/data', image, user: '00:1' },
      { root: '/private/data', image, user: '1:0' }, { root: '/private/data', image, privileged: true },
    ...
    expectTypeOf(dockerSandbox({ root: '/private/data', image }).capabilities).toEqualTypeOf<readonly ['sandbox.fs', 'sandbox.exec', 'sandbox.spawn', 'sandbox.persistent_fs'] | undefined>()
  })
  ...
  it('rejects unsupported restore, invalid scope, and pre-cancelled open without CLI mutation', async () => {
    ...
    await expect(adapter.open({ scope: { ...scope, partition: { kind: 'unknown' } } as unknown as SandboxScope, mode: 'create' })).rejects.toBeInstanceOf(ValidationError)
    await expect(adapter.open({ scope, mode: 'create', signal: AbortSignal.abort('private') })).rejects.toBeInstanceOf(OperationCancelledError)
    expect(transport.calls).toEqual([])
  })
  ...
  it('fails closed through the adapter facade when its durable owner journal is truncated', async () => {
    ...
    await expect(resumed.registerOwner({ owner: scope.owner, mode: 'attach' })).rejects.toBeInstanceOf(SandboxStateLostError)
    expect(transport.calls).toEqual([])
  })
  ...
  it('emits a redacted register-owner operation through the public adapter facade', async () => {
    ...
    adapter.configureHarnessContext({ telemetry, defaults: { toolTimeoutMs: 1_000 } } as unknown as HarnessAdapterContext)

    await adapter.registerOwner({ owner: scope.owner, mode: 'create' })

    expect(telemetry.spans).toEqual([expect.objectContaining({
...
describe('Docker lifecycle protocol', () => {
  it('exposes only owner-scoped administrative inventory and purges its exact private resources', async () => {
    ...
    await expect(adapter.administration.purge({ selector: { kind: 'owner', owner: scope.owner }, idempotencyKey: 'purge-owner' }))
  ...
  it('fences an already-open sibling partition as soon as owner purge commits revocation', async () => {
    ...
    const result = await adapter.administration.purge({ selector: { kind: 'owner', owner: scope.owner }, idempotencyKey: 'purge-owner', limit: 1 })
    ...
    expect(attempts.some(attempt => attempt.status === 'rejected'
      && typeof attempt.reason === 'object'
      && attempt.reason !== null
      && 'meta' in attempt.reason
      && typeof attempt.reason.meta === 'object'
      && attempt.reason.meta !== null
      && attempt.reason.meta['reason'] === 'owner_revoked')).toBe(true)
  })

  it('revokes only the offboarded principal from an already-open tenant-owned partition', async () => {
    const { adapter } = await fixture()
    const owner = { ...scope.owner, id: 'tenant-shared', instanceId: '01JQ7Z9Q69STZ33MGH6V5ASR7K', identity: { tenantId: 'tenant-a' } }
    const sharedScope: SandboxScope = { ...scope, owner }
    scopes.set(adapter, [sharedScope])
    await adapter.registerOwner({ owner, mode: 'create' })
    const first = await adapter.open({ scope: sharedScope, mode: 'create', identity: { tenantId: 'tenant-a', principalId: 'principal-a' } })
    const second = await adapter.open({ scope: sharedScope, mode: 'create', identity: { tenantId: 'tenant-a', principalId: 'principal-b' } })

    await expect(adapter.administration.purge({
    ...
    await expect(first.session.write('/workspace/revoked', 'x')).rejects.toMatchObject({ meta: { reason: 'principal_revoked' } })
  ...
  it('creates once with secure defaults; shares same-client attachments and reattaches after release', async () => {
    ...
    await adapter.registerOwner({ owner: scope.owner, mode: 'create' })
    const [first, second] = await Promise.all([adapter.open({ scope, mode: 'create' }), adapter.open({ scope, mode: 'create' })])
    ...
    expect(runs[0]).toEqual(expect.arrayContaining(['--pull', 'never', '--network', 'none', '--user', '1000:1000', '--cap-drop', 'ALL', '--security-opt', 'no-new-privileges:true', '--cpus', '1', '--memory', '512m', '--pids-limit', '128']))
    ...
    const allState = await Promise.all((await readdir(join(root, 'docker-sandbox'))).map(file => readFile(join(root, 'docker-sandbox', file), 'utf8')))
  ...
  it('rejects independent concurrent owners without changing resources', async () => {
    ...
    const client = new DockerSandbox({ root, image }, transport)
    await client.registerOwner({ owner: scope.owner, mode: 'attach' })
    await expect(client.open({ scope, mode: 'attach' })).rejects.toMatchObject({ meta: { reason: 'ownership_conflict' } })
    await expect(client.terminate({ scope, reason: 'manual' })).rejects.toMatchObject({ meta: { reason: 'ownership_conflict' } })
  ...
  it('retains terminal intent through partial cleanup and retries only its exact resources', async () => {
    const { adapter, transport } = await fixture()
    const { session } = await open(adapter)
    transport.failure = args => args[0] === 'volume' && args[1] === 'rm' ? { code: 1, stderr: 'private daemon details' } : undefined
  ...
  it('keeps unrelated Docker cleanup failures pending', async () => {
    const { adapter, transport } = await fixture()
    await (await open(adapter)).session.close()
    transport.failure = args => args[0] === 'container' && args[1] === 'stop'
    ...
    await expect(adapter.terminate({ scope, reason: 'manual' })).rejects.toMatchObject({
      code: 'SANDBOX_ERROR', meta: { reason: 'guest_cleanup_failed' }
    })
  })
  ...
  it.each(['container', 'volume', 'metadata'] as const)('does not replace missing %s', async missing => {
    ...
    if (missing === 'metadata') {
      const record = (await readdir(join(root, 'docker-sandbox'))).find(name => name.endsWith('.json') && !name.endsWith('.owner.json'))!
  ...
  it('does not classify outage, permissions, or a changed engine as state loss', async () => {
    ...
    const { session } = await open(adapter)
    await session.close()
    transport.failure = args => args[1] === 'inspect' && args[0] === 'container' ? { code: 1, stderr: 'permission denied at private socket' } : undefined
    ...
    await expect(adapter.open({ scope, mode: 'attach' })).rejects.toMatchObject({ meta: { reason: 'engine_identity_changed' } })
  })
  ...
  it('pins the Unix endpoint rather than following subsequent active context changes', async () => {
    ...
    expect(transport.calls.filter(call => call[0] === '--host').every(call => call[1] === 'unix:///test-docker.sock')).toBe(true)
  })

  it('rejects changed image or resource policy on attach while preserving explicit cleanup', async () => {
    const { adapter, transport, root } = await fixture()
    await (await open(adapter)).session.close()
    for (const overrides of [{ network: 'bridge' as const }, { user: '2000:2000' }, { image: `sha256:${'b'.repeat(64)}` }, { resources: { pids: 256 } }]) {
      const changed = new DockerSandbox({ root, image, ...overrides }, transport)
      await changed.registerOwner({ owner: scope.owner, mode: 'attach' })
      await expect(changed.open({ scope, mode: 'attach' })).rejects.toMatchObject({ meta: { reason: 'sandbox_policy_changed' } })
    }
    expect(transport.containers.size).toBe(1)
  ...
  it('stops retained guest work before granting a fresh owner when owner metadata is absent', async () => {
    ...
    expect(calls.findIndex(call => call.includes('stop'))).toBeLessThan(calls.findIndex(call => call.includes('start')))
    await attached.session.close()
  })
  ...
  it('rejects remote engines and missing image utilities before accepting a sandbox', async () => {
    ...
    second.transport.failure = args => args.some(arg => arg.includes('for tool in')) ? { code: 1, stderr: 'private path' } : undefined
  ...
  it('recovers a confirmed dead owner only after stopping retained guest work', async () => {
    ...
    await writeFile(join(directory, record.replace('.json', '.owner')), JSON.stringify({ pid: 2_147_483_647, token: '715bcebf-e48f-4976-8e74-f61d6d1ba9dc' }))
    transport.containers.values().next().value!.running = true
    const before = transport.calls.length
...
describe('Docker guest operations', () => {
  it('roundtrips binary data and delimiter-containing filenames without leaking metadata', async () => {
    ...
    expect(await session.list('/workspace')).toEqual([{ path: name, name: 'line\nwith\ttabs.bin', kind: 'file', size: 4 }])
  ...
  it('returns guest nonzero exit codes and stderr without putting content in errors', async () => {
    const { adapter, transport } = await fixture()
    const { session } = await open(adapter)
    transport.failure = args => args.includes('fail-command') ? { stdout: 'user result', stderr: 'user stderr', code: 23 } : undefined
    await expect(session.exec('fail-command')).resolves.toMatchObject({ stdout: 'user result', stderr: 'user stderr', exitCode: 23 })
  })
  ...
  it('bounds output and confirms guest cleanup instead of silently truncating', async () => {
    const { adapter, transport } = await fixture()
    const { session } = await open(adapter)
    transport.failure = args => args.includes('too-much') ? { stdout: Buffer.alloc(OUTPUT_LIMIT_BYTES + 1) } : undefined
  ...
  it('fails cleanup visibly and retains ownership until an explicit successful close retry', async () => {
    ...
    await expect(other.open({ scope, mode: 'attach' })).rejects.toMatchObject({ meta: { reason: 'ownership_conflict' } })
    transport.failure = undefined
    await session.close()
  ...
  it('honors explicit SIGKILL and confirms guest exit before resolving kill', async () => {
    ...
    await expect(process.exit).resolves.toMatchObject({ exitCode: 137 })
    expect(transport.containers.values().next().value!.running).toBe(false)
    expect(transport.calls.some(call => call.includes('kill') && call.includes('--signal') && call.includes('KILL'))).toBe(true)
    await process.kill('SIGKILL')
  ...
  it('decodes split UTF-8 sequences and preserves per-stream output bounds', async () => {
    ...
    const child = { stdout: (async function* () { yield bytes.slice(0, 1); yield bytes.slice(1) })(), stderr: (async function* () {})(), exit: Promise.resolve({ exitCode: 0 }), end() {}, kill() {}, async write() {} }
