...
import { AgentLoopBudgetError, DecisionBlockedError, DecisionEvaluationError, HarnessConfigError, HarnessError, OperationCancelledError, OperationTimeoutError, SkillManifestError, ValidationError, serializeError } from '../errors/index.js'
import type { Logger } from '../logger/index.js'
import { isJsonValue, type JsonValue } from '../models/json.js'
import type { Message } from '../models/state.js'
import type { AgentDefinition, AgentExecutionInterception, AgentExecutionInterceptor, AgentModelRequest, BuiltinToolName, GovernanceConfig, ModelHandles, ResolvedSkill, RunEvent, ToolsConfig } from '../harness/defineHarness.js'
...
import { createDecisionEvidence, decisionResultSchema, providerContinuationSchema, runDecisionOperation } from '../decisions/index.js'
...
async function runDefaultAgentInner(args: {
  ...
  if (!isJsonValue(parsedInput)) throw new ValidationError('Agent input validation failed.', { where: 'agent_input', issues: [] })
  ...
  const tsCustomSpecs = Object.entries(args.customTools)
    ...
    .flatMap(([name, tool]) => {
      ...
      return [{ name, description: tsTool.description, parameters: z.toJSONSchema(tsTool.input, { io: 'input' }) as JsonValue }]
    })
  const mcpSpecs = args.mcpRegistry ? await getMcpToolSpecs(args.customTools, enabledCustomTools, { registry: args.mcpRegistry, signal: args.signal, toolTimeoutMs: args.toolTimeoutMs, sandbox: args.session, sandboxKey: args.sandboxKey ?? args.sessionId }) : []
  ...
  try {
    while (true) {
      ...
      let request: AgentModelRequest = {
        ...
        tools: stepTools,
        schema: z.toJSONSchema(outputSchema, { io: 'input' }) as JsonValue,
      ...
      if (await shouldStopAgentLoop(args, parsedInput, stepModelAlias, steps, modelMessages, allToolSpecs, response as ObjectResponse<JsonValue>, toolCalls)) {
        const candidate = await applyBeforeOutputInterceptors(args, parsedInput, steps, stepModelAlias, response.object as JsonValue)
        const validated = parseAgentSchema(outputSchema, candidate, 'agent_output')
        await args.emitEvent?.({ type: 'model.object', runId: args.runId, agentId: args.agentId, ...(args.workflowId ? { workflowId: args.workflowId } : {}), modelAlias: stepModelAlias, object: validated as JsonValue })
      ...
      const batch = await runPreparedToolBatch({
        ...
        beforeTool: (toolId, callId, value, signal, deadline) => applyBeforeToolInterceptors(args, parsedInput, steps, stepModelAlias, toolId, callId, value, signal, deadline),
        afterTool: (toolId, callId, value, signal, deadline) => applyAfterToolInterceptors(args, parsedInput, steps, stepModelAlias, toolId, callId, value, signal, deadline)
      }, toolCalls, stepTools)
      const effectiveToolCalls = batch.calls
      const assistantMsg: Message = {
        id: turnMessageId(args.delegationCallId ?? args.runId, `10_assistant_${steps}`), sessionId: args.sessionId, runId: args.runId, role: 'assistant', content: '', toolCalls: [...effectiveToolCalls],
      ...
      modelMessages.push({ role: 'assistant', content: assistantMsg.content, toolCalls: [...effectiveToolCalls], ...(response.providerContinuation ? { providerContinuation: response.providerContinuation } : {}) })
...
const interceptorHooks = { before_input: 'beforeInput', before_model: 'beforeModel', after_model: 'afterModel', before_tool: 'beforeTool', after_tool: 'afterTool', before_output: 'beforeOutput' } as const
...
async function applyInterceptors<T>(
  ...
  value: T,
  invoke: (interceptor: AgentExecutionInterceptor<any, any>, current: T, decision: DecisionExecutionContext) => AgentExecutionInterception<T> | void | Promise<AgentExecutionInterception<T> | void>,
  execution: DecisionExecutionContext = { signal: args.signal, deadline: args.runDeadline ?? Number.POSITIVE_INFINITY },
  allowTransform = true,
  step = 0,
  onTransform?: (value: JsonValue, evidence: ReturnType<typeof createDecisionEvidence>) => T,
  ...
  for (const interceptor of args.agent.interceptors ?? []) {
    ...
    const evidence = createDecisionEvidence({
      occurrence: { invocationId: args.delegationCallId ?? args.runId, runId: args.runId, agentId: args.agentId, sessionId: args.sessionId, ...(args.workflowId ? { workflowId: args.workflowId } : {}), ...toolOccurrence, step },
      source: { kind: 'interceptor', id: interceptor.id }, phase: evidencePhase(phase), ordinal
    })
    ordinal += 1
    const deadline = Date.now() + args.decisionTimeoutMs
    try {
      outcome = await runDecisionOperation({ signal: execution.signal, deadline }, (signal) => invoke(interceptor, freezeJson(current), { signal, deadline: Math.min(deadline, execution.deadline) }))
    } catch (error) {
      if (error instanceof OperationTimeoutError && error.meta?.['scope'] === 'decision') throw new DecisionEvaluationError(evidence, 'callback_timeout', error)
      if (error instanceof DecisionBlockedError || error instanceof DecisionEvaluationError || error instanceof OperationCancelledError || error instanceof OperationTimeoutError) throw error
    ...
    if (result.decision === 'transform' && allowTransform) {
      ...
      try {
        current = onTransform(result.value, evidence)
      } catch (error) {
    ...
    if (result.decision === 'block') {
      throw new DecisionBlockedError(createDecisionEvidence({
        occurrence: { invocationId: args.delegationCallId ?? args.runId, runId: args.runId, agentId: args.agentId, sessionId: args.sessionId, ...(args.workflowId ? { workflowId: args.workflowId } : {}), ...toolOccurrence, step },
        source: { kind: 'interceptor', id: interceptor.id }, phase: evidencePhase(phase), ordinal: ordinal - 1,
...
async function applyBeforeInputInterceptors(args: DefaultAgentArgs, input: unknown): Promise<unknown> {
  ...
  return applyInterceptors(args, 'before_input', input as JsonValue, (interceptor, current, decision) => interceptor.beforeInput?.({
    ...interceptorContext(args, interceptor.id, current, 0, args.modelAlias ?? args.agent.model, decision),
    input: current
  }), undefined, true, 0, (value, evidence) => {
    const parsed = parseAgentSchema(schema, value, 'agent_input')
    if (!isJsonValue(parsed)) throw new DecisionEvaluationError(evidence, 'invalid_transform')
    return parsed
  })
}
...
async function applyBeforeModelInterceptors(
  ...
  const transformed = await applyInterceptors(args, 'before_model', { messages: request.messages }, (interceptor, current, decision) => interceptor.beforeModel?.({
    ...interceptorContext(args, interceptor.id, input, step, model, decision),
    request: freezeJson({ ...request, messages: current.messages })
  }), undefined, true, step, (value, evidence) => {
...
async function applyAfterModelInterceptors(
  ...
  await applyInterceptors(args, 'after_model', response, (interceptor, _current, decision) => interceptor.afterModel?.({
    ...interceptorContext(args, interceptor.id, input, step, model, decision),
    request: freezeJson(request),
    response: freezeJson(response)
  }), undefined, false, step)
  return response
}

async function applyBeforeToolInterceptors(args: DefaultAgentArgs, agentInput: unknown, step: number, model: string, toolId: string, callId: string, input: JsonValue, signal: AbortSignal, deadline: number): Promise<JsonValue> {
  return applyInterceptors(args, 'before_tool', input, (interceptor, current, decision) => interceptor.beforeTool?.({
    ...interceptorContext(args, interceptor.id, agentInput, step, model, decision),
    toolId,
    callId,
    input: current
  }), { signal, deadline }, true, step, (value) => value, { toolId, callId })
}

async function applyAfterToolInterceptors(args: DefaultAgentArgs, agentInput: unknown, step: number, model: string, toolId: string, callId: string, output: JsonValue, signal: AbortSignal, deadline: number): Promise<JsonValue> {
  return applyInterceptors(args, 'after_tool', output, (interceptor, current, decision) => interceptor.afterTool?.({
    ...interceptorContext(args, interceptor.id, agentInput, step, model, decision),
    toolId,
    callId,
    output: current
...
async function applyBeforeOutputInterceptors(args: DefaultAgentArgs, agentInput: unknown, step: number, model: string, output: JsonValue): Promise<JsonValue> {
  return applyInterceptors(args, 'before_output', output, (interceptor, current, decision) => interceptor.beforeOutput?.({
    ...interceptorContext(args, interceptor.id, agentInput, step, model, decision),
    output: current
  }), undefined, true, step, (value) => value)
}
...
function prepareStepEvidence(args: DefaultAgentArgs, step: number): ReturnType<typeof createDecisionEvidence> {
  return createDecisionEvidence({
    occurrence: { invocationId: args.delegationCallId ?? args.runId, runId: args.runId, agentId: args.agentId, sessionId: args.sessionId, ...(args.workflowId ? { workflowId: args.workflowId } : {}), step },
...
function evidencePhase(phase: InterceptorPhase): 'input' | 'before_model' | 'after_model' | 'tool_input' | 'tool_output' | 'output' {
...
function isModelRequestTransform(value: unknown): value is { messages: readonly ModelMessage[] } {
  ...
  return Object.keys(record).length === 1 && Array.isArray(record['messages']) && record['messages'].every(isStrictModelMessage)
}

function isStrictModelMessage(value: unknown): value is ModelMessage {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return false
  const record = value as Record<string, unknown>
  if (record['role'] === 'system' || record['role'] === 'user') return isStrictContent(record['content']) && Object.keys(record).every((key) => key === 'role' || key === 'content')
  if (record['role'] === 'assistant') return isStrictContent(record['content']) && (record['toolCalls'] === undefined || (Array.isArray(record['toolCalls']) && record['toolCalls'].every(isStrictToolCall))) && (record['providerContinuation'] === undefined || providerContinuationSchema.safeParse(record['providerContinuation']).success) && Object.keys(record).every((key) => key === 'role' || key === 'content' || key === 'toolCalls' || key === 'providerContinuation')
  return record['role'] === 'tool' && typeof record['toolCallId'] === 'string' && typeof record['content'] === 'string' && Object.keys(record).every((key) => key === 'role' || key === 'toolCallId' || key === 'content')
}
...
function isStrictContent(value: unknown): boolean {
  ...
  return value.every((part) => {
    ...
    if (record['kind'] === 'text') return typeof record['text'] === 'string' && Object.keys(record).every((key) => key === 'kind' || key === 'text')
    if (record['kind'] === 'image' || record['kind'] === 'audio') return typeof record['mimeType'] === 'string' && typeof record['dataBase64'] === 'string' && Object.keys(record).every((key) => key === 'kind' || key === 'mimeType' || key === 'dataBase64')
    if (record['kind'] === 'image_url') return typeof record['url'] === 'string' && (record['mimeType'] === undefined || typeof record['mimeType'] === 'string') && Object.keys(record).every((key) => key === 'kind' || key === 'url' || key === 'mimeType')
    if (record['kind'] === 'file') return typeof record['mimeType'] === 'string' && typeof record['dataBase64'] === 'string' && (record['filename'] === undefined || typeof record['filename'] === 'string') && Object.keys(record).every((key) => key === 'kind' || key === 'mimeType' || key === 'dataBase64' || key === 'filename')
    if (record['kind'] === 'file_url') return typeof record['url'] === 'string' && (record['mimeType'] === undefined || typeof record['mimeType'] === 'string') && (record['filename'] === undefined || typeof record['filename'] === 'string') && Object.keys(record).every((key) => key === 'kind' || key === 'url' || key === 'mimeType' || key === 'filename')
    return false
  })
}
...
function isStrictToolCall(value: unknown): value is ToolCallSpec {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return false
  const record = value as Record<string, unknown>
  return typeof record['id'] === 'string' && typeof record['name'] === 'string' && isJsonValue(record['arguments']) && Object.keys(record).every((key) => key === 'id' || key === 'name' || key === 'arguments')
}

function assertProtectedTranscript(canonical: readonly ModelMessage[], candidate: readonly ModelMessage[]): void {
  if (!candidate.every(isStrictModelMessage)) throw new ValidationError('Model messages are invalid.', { where: 'model_response', issues: [] })
  const groups = toolInteractionGroups(canonical)
  const retainedResultIndices = new Set<number>()
  const ids = new Set<string>()
  for (const message of candidate) if (message.role === 'assistant' && message.toolCalls) for (const call of message.toolCalls) {
    if (ids.has(call.id)) throw new ValidationError('Model messages contain duplicate tool call ids.', { where: 'model_response', issues: [] })
    ids.add(call.id)
  }
  for (const [groupIndex, group] of groups.entries()) {
    const index = candidate.findIndex((message) => structurallyEqual(message, group.assistant))
    if (index < 0) {
      if (groupIndex === groups.length - 1) throw new ValidationError('The latest tool interaction group must be retained.', { where: 'model_response', issues: [] })
      continue
    }
    if (!group.results.every((result, offset) => structurallyEqual(candidate[index + offset + 1], result))) throw new ValidationError('Protected tool interaction groups cannot be rewritten.', { where: 'model_response', issues: [] })
  ...
  for (const [index, message] of candidate.entries()) {
    if (message.role === 'assistant' && message.toolCalls && !groups.some((group) => structurallyEqual(message, group.assistant))) throw new ValidationError('Protected tool interaction groups cannot be injected.', { where: 'model_response', issues: [] })
    if (message.role === 'tool' && !retainedResultIndices.has(index)) throw new ValidationError('Protected tool results must belong to one complete retained interaction group.', { where: 'model_response', issues: [] })
  }
}

function toolInteractionGroups(messages: readonly ModelMessage[]): Array<{ assistant: ModelMessage; results: ModelMessage[] }> {
  ...
  for (let index = 0; index < messages.length; index += 1) {
    ...
    if (results.length !== callIds.size) throw new ValidationError('Protected tool interaction group is incomplete.', { where: 'model_response', issues: [] })
    groups.push({ assistant, results })
  }
  return groups
}

function structurallyEqual(left: unknown, right: unknown): boolean { return JSON.stringify(left) === JSON.stringify(right) }
