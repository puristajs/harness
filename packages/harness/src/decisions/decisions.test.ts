...
describe('decision foundation', () => {
  it('creates frozen deterministic content-free evidence from strict identity fields', () => {
    const evidence = createDecisionEvidence({ occurrence, source, phase: 'policy', ordinal: 0, reasonCode: 'amount_limit' })
    ...
    expect(createDecisionEvidence({ occurrence, source, phase: 'policy', ordinal: 1 }).decisionId).not.toBe(evidence.decisionId)
    expect(createDecisionEvidence({ occurrence: { ...occurrence, callId: 'call-2' }, source, phase: 'policy', ordinal: 0 }).decisionId).not.toBe(evidence.decisionId)
  })
  ...
  it('rejects malformed closed records and safe-field violations without candidate data', () => {
    ...
    try {
      ...
      expect(error).toMatchObject({ code: 'HARNESS_CONFIG_ERROR', message: 'Decision evidence configuration is invalid.' })
    }
    expectDecisionConfigFailure(() => createDecisionEvidence({
      occurrence,
      source,
      phase: 'policy',
      ordinal: 0,
  ...
  it('projects explicit blocks and failed evaluation through fixed safe errors', () => {
    ...
    expect(new DecisionBlockedError(evidence)).toMatchObject({
      code: 'DECISION_BLOCKED', category: 'interceptor', retriable: false, message: 'Decision blocked execution.', meta: { evidence }
    })
    expect(new DecisionEvaluationError(evidence, 'callback_timeout')).toMatchObject({
      code: 'DECISION_EVALUATION_ERROR', category: 'interceptor', retriable: false, message: 'Decision evaluation failed closed.', meta: { evidence, failureKind: 'callback_timeout' }
    })
  ...
  it('runs a callback once with a child signal and cleans up after resolution', async () => {
    ...
    await expect(runDecisionOperation({ signal: parent.signal, deadline: Date.now() + 100 }, operation)).resolves.toBe('allowed')
  ...
  it('does not invoke protected work for pre-aborted or expired contexts', async () => {
    ...
    await expect(runDecisionOperation({ signal: parent.signal, deadline: Date.now() + 100 }, operation)).rejects.toMatchObject({ code: 'OPERATION_CANCELLED' })
    await expect(runDecisionOperation({ signal: new AbortController().signal, deadline: Date.now() - 1 }, operation)).rejects.toMatchObject({
  ...
  it('fences late callback completion and preserves parent cancellation', async () => {
    ...
    try {
      ...
      const cancelling = runDecisionOperation({ signal: cancelled.signal, deadline: Date.now() + 100 }, () => new Promise(() => {}))
