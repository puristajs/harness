...
describe('compiled agent execution requirements', () => {
  it('merges direct declarations deterministically without weakening their provenance', () => {
    expect(compileAgentExecutionRequirements([
      { path: 'interceptors.first', requirements: { tools: ['search', 'search'], models: [{ alias: 'primary', capabilities: ['text', 'tool_use'] }] } },
      { path: 'interceptors.second', requirements: { tools: ['write', 'search'], models: [{ alias: 'primary', capabilities: ['tool_use', 'object'] }, { alias: 'reviewer', capabilities: ['text'] }] } },
      { path: 'interceptors.empty', requirements: {} }
    ])).toEqual({
      tools: [{ id: 'search', path: 'interceptors.first' }, { id: 'write', path: 'interceptors.second' }],
      models: [
        { alias: 'primary', path: 'interceptors.first', capabilities: [
