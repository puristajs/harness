      ...
      wait: {
        ...
        handler: async (ctx) => {
          await ctx.externalWait.wait({
            waitId, kind: 'human_review', schemaVersion: 'v1', definitionVersion: 'v1', deadline: '2030-01-01T00:00:00.000Z'
          })
          return 'unreachable'
        }
      }
...
describe('durable external waits', () => {
  it('uses strict request, signal, and discriminated snapshot shapes', () => {
    ...
    expect(() => validateExternalWaitRequest({ ...request, extra: true })).toThrow(expect.objectContaining({ reason: 'invalid_request' } satisfies Partial<ExternalWaitError>))
    expect(() => validateExternalWaitRequest({ ...request, deadline: '2030-01-01T00:00:00Z' })).toThrow(expect.objectContaining({ reason: 'invalid_request' } satisfies Partial<ExternalWaitError>))
    expect(() => validateExternalWaitSignal({ waitId: 'wait-1', eventId: 'event-1', outcome: 'approved', observedAt: '2030-01-01T00:00:00.000Z', extra: true })).toThrow(expect.objectContaining({ reason: 'invalid_request' } satisfies Partial<ExternalWaitError>))
    expect(() => validateExternalWaitSnapshot({ ...request, status: 'approved', createdAt: request.deadline, resolvedAt: request.deadline })).toThrow(expect.objectContaining({ reason: 'invalid_snapshot' } satisfies Partial<ExternalWaitError>))
    expect(() => validateExternalWaitSnapshot({ ...request, status: 'expired', createdAt: request.deadline, resolvedAt: request.deadline, eventId: undefined })).toThrow(expect.objectContaining({ reason: 'invalid_snapshot' } satisfies Partial<ExternalWaitError>))
  })
  ...
  it('expires against the injected clock before a signal and retains the automatic-expiry shape', async () => {
    ...
    await storage.createRun({ id: 'run-at-deadline', sessionId: 'session-at-deadline', kind: 'workflow', target: 'wait', startedAt: now.toISOString(), status: 'running' })
    await storage.acquireRun({ runId: 'run-at-deadline', sessionId: 'session-at-deadline', workerId: 'worker', stepId: 'wait', input: null })
    ...
    await expect(storage.signalWait({ waitId: 'wait-at-deadline', eventId: 'late-delivery', outcome: 'approved' })).resolves.toEqual({ kind: 'duplicate', snapshot: late.snapshot })
    await expect(storage.getWait('')).rejects.toMatchObject({ reason: 'invalid_request' })
    await expect(storage.cancelWait('wait-at-deadline', '', now.toISOString())).rejects.toMatchObject({ reason: 'invalid_request' })
  })
  ...
  it('rejects a malformed adapter readback before emitting external-wait events', async () => {
    ...
    await expect(session.workflows.wait.prompt('x', { durable: { runId: 'invalid-readback-run' } })).rejects.toMatchObject({ reason: 'invalid_snapshot' })
    expect((await storage.listEvents('invalid-readback-run')).filter((event) => event.type.startsWith('external_wait.'))).toEqual([])
  })
  ...
  it('rejects malformed and foreign getWait readbacks before requested or resolved events', async () => {
    ...
    for (const [storage, waitId, runId, sessionId] of [
      ...
      await expect(session.workflows.wait.prompt('x', { durable: { runId } })).rejects.toMatchObject({ reason: 'invalid_snapshot' })
