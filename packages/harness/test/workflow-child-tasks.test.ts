...
describe('workflow child tasks', () => {
  it('starts an isolated, workflow-owned task that can settle after its starter workflow', async () => {
    ...
    provider.enqueueObject({ object: 'done', usage: { inputTokens: 1, outputTokens: 1, totalTokens: 2 }, finishReason: 'stop' })
    let handle: ChildTaskHandle<string> | undefined
    const harness = defineHarness()
      .sandbox(inMemorySandbox())
      .models({ fake: { provider, model: 'fake', capabilities: ['object'] } })
      .agents(({ agent }) => ({
        worker: agent({ model: 'fake', input: z.string(), output: z.string(), builtinTools: false, instructions: 'Return done.' })
      }))
      .workflows(({ workflow }) => ({
        launch: workflow({
    ...
    const taskId = await session.workflows.launch.prompt('private parent context')
    expect(handle?.id).toBe(taskId)
    await expect(handle?.result()).resolves.toBe('done')
    await expect(handle?.status()).resolves.toMatchObject({ status: 'succeeded', descriptor: { contextPolicy: 'isolated', parentRunId: expect.any(String) } })
    ...
    expect(provider.requests[0]).toMatchObject({ messages: expect.not.arrayContaining([expect.objectContaining({ content: 'older private parent secret' })]) })
  ...
  it('cancels a live task without cancelling a later workflow invocation', async () => {
    ...
    const harness = defineHarness()
      ...
      .agents(({ agent }) => ({
        worker: agent({
          model: 'fake', input: z.string(), output: z.string(), builtinTools: false, instructions: 'Wait.',
          handler: async (ctx) => new Promise<string>((_resolve, reject) => {
      ...
      .workflows(({ workflow }) => ({
        ...
        healthy: workflow({ input: z.string(), output: z.string(), handler: async (ctx) => ctx.input })
      }))
      .build()

    const session = await harness.getSession('task-cancel')
    const taskId = await session.workflows.launch.prompt('work')
    // The task is deliberately independent of its completed starter workflow.
    await expect(session.workflows.healthy.prompt('next')).resolves.toBe('next')
  ...
  it('queues background tasks under the delegation ceiling instead of rejecting them', async () => {
    ...
    const harness = defineHarness()
      ...
      .workflows(({ workflow }) => ({
        launch: workflow({
          input: z.array(z.string()), output: z.array(z.string()),
    ...
    await session.workflows.launch.prompt(['one', 'two'])
  ...
  it('keeps a continuable task-owned history and exposes it through the session owner', async () => {
    ...
    const harness = defineHarness()
      ...
      .agents(({ agent }) => ({
        worker: agent({
          model: 'fake', input: z.string(), output: z.string(), builtinTools: false, instructions: 'Echo.',
    ...
    const taskId = await session.workflows.launch.prompt('first')
    ...
    await expect(session.childTasks.list()).resolves.toContainEqual(expect.objectContaining({ status: 'succeeded', descriptor: expect.objectContaining({ mode: 'continuable' }) }))
    await harness.shutdown()
  })
})
