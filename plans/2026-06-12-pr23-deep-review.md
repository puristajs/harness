...
### Follow-up fixes — 2026-06-12 (later in the same session)

- **P1 — Durable workspace idempotency conflicts now enforced for every op (not just `start`).** `pauseWorkspace`, `resumeWorkspace`, and `abortWorkspace` previously replayed a stored result by `idempotencyKey` without validating operation kind or run/session identity — a key reused under a different run/session silently replayed the stale result, and a cross-kind key collision replayed a wrong-typed result (e.g. a `WorkspaceCheckpoint` cast as a `WorkspaceHandle`). Both stores now validate kind + `runId`/`sessionId` on replay and throw `WorkspaceError{meta.reason:'idempotency_conflict'}` on mismatch: `local-workspace.ts` via a shared `assertReplayMatches` guard (the persisted op already carried `{kind,runId,sessionId,result}`); `in-memory.ts` by widening `opResults` to `{kind,runId,sessionId,workspaceRef,result}` (was a bare global `Map<string,unknown>` shared across all workspaces). `durableWorkspaceStoreContract.ts` gained four conflict cases (pause/resume/abort identity reuse + pause→resume cross-kind), run against both stores. `specs/22-local-durable-execution.md` §4 reworded to state replay validates kind+identity for all four durable ops. Verified: typecheck + 402 harness tests + coverage gates green.
- **P3 — `relayRunEvents` stream queue is now genuinely bounded.** Only `run.finished` (≤1 per run) was meant to be undroppable, but `agent.finished` (emitted once per child-agent delegation call, budget-configurable) was also in the never-drop set, so a slow consumer on a delegation-heavy run could grow the queue past `STREAM_MAX_BUFFERED_EVENTS` (`dropIndex === -1` still pushed). Narrowed the set to `run.finished` only (`STREAM_TERMINAL_EVENT_TYPES` → `STREAM_UNDROPPABLE_EVENT_TYPES`) and added a defensive fallback that discards-and-counts the incoming event when every queued event is undroppable. Doc comment corrected; regression test added in `test/relayRunEvents.test.ts` (3072 `agent.finished` events + a slow consumer; asserts the buffer stays ≤ cap, `run.finished` is always delivered, and drop accounting balances). Verified green.
