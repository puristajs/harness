---
id: "TICKET-007"
title: "Retry-safe application approval claim"
wave: 7
lifecycle: "accepted"
...
parallel_group: "decision-sequential-7"
depends_on:
  - "TICKET-006"
blocked_by: []
spec_refs:
  - "ai-harness/specs/37-decision-boundaries/03-contracts/decisions.md#CTR-DB-REVIEW"
  - "ai-harness/specs/37-decision-boundaries/03-flows/e2e-coverage.md#REQ-DB-REVIEW"
...
traceability:
  ...
  capability_ids:
    - "CAP-DB-REVIEW"
  path_ids:
    - "PATH-DB-REVIEW-SUCCESS"
    - "PATH-DB-REVIEW-FAILURE"
    - "PATH-DB-REVIEW-RECOVERY"
  acceptance_ids:
    - "AC-DB-REVIEW-SUCCESS"
    - "AC-DB-REVIEW-FAILURE"
    - "AC-DB-REVIEW-RECOVERY"
write_scope:
  - "ai-harness/examples/durable-human-review"
  - "ai-harness/packages/harness/src/sessions/index.ts"
  - "ai-harness/packages/harness/src/storage/types.ts"
  - "ai-harness/packages/harness/src/storage/in-memory.ts"
  - "ai-harness/packages/harness/test/durable-session.test.ts"
read_scope:
...
verification_commands:
  ...
  CMD-CORE-BUILD:
    ...
    purpose: "requirement-scoped verification for REVIEW"
    expected: "pass"
    network: "forbidden"
...
action_steps:
  ...
  - id: "STEP-IMPLEMENT"
    ...
    acceptance_refs:
      ...
      - "ACC-REVIEW-FAILURE"
      - "ACC-REVIEW-RECOVERY"
    expected_proof: "Replace consumed/consumeApproved with exact local ReviewTask/PaymentAction/ExecutionRecord/Receipt schemas and methods. Implement frozen creation expiry, bounded deterministic identities, CAS decision/claim/receipt state transitions, injected auth/revision/clock, idempotent reference executor. Bind invocation action before replay-skipped step; existing claim/receipt bypass new admission checks. Remove unsafe checkpoint casts and fixed 2030 deadline. Keep review CRUD/business identity outside Harness."
  - id: "STEP-VERIFY"
    kind: "verify"
    files:
      - "ai-harness/examples/durable-human-review"
...
acceptance:
  - id: "ACC-REVIEW-SUCCESS"
    traceability_acceptance_ids:
      - "AC-DB-REVIEW-SUCCESS"
    requirement_refs:
      - "ai-harness/specs/37-decision-boundaries/03-flows/e2e-coverage.md#REQ-DB-REVIEW"
    test_refs:
      - "ai-harness/examples/durable-human-review/src/payment-review.test.ts"
    command_refs:
      - "CMD-CORE-BUILD"
      - "CMD-REVIEW"
      - "CMD-REVIEW-TYPES"
      - "CMD-WAITS"
    expected_outcome: "Approved action records one admitted execution and receipt"
    lifecycle: "accepted"
  - id: "ACC-REVIEW-FAILURE"
    traceability_acceptance_ids:
      - "AC-DB-REVIEW-FAILURE"
    requirement_refs:
      - "ai-harness/specs/37-decision-boundaries/03-flows/e2e-coverage.md#REQ-DB-REVIEW"
    ...
    command_refs:
      ...
      - "CMD-REVIEW-TYPES"
      - "CMD-WAITS"
    expected_outcome: "Unauthorized expired changed or conflicting action has no new effect"
    lifecycle: "accepted"
  - id: "ACC-REVIEW-RECOVERY"
    traceability_acceptance_ids:
      - "AC-DB-REVIEW-RECOVERY"
    requirement_refs:
      - "ai-harness/specs/37-decision-boundaries/03-flows/e2e-coverage.md#REQ-DB-REVIEW"
    ...
    command_refs:
      ...
      - "CMD-REVIEW-TYPES"
      - "CMD-WAITS"
    expected_outcome: "Every claim effect receipt checkpoint crash window resumes same execution identity"
    lifecycle: "accepted"
---

# TICKET-007 — Retry-safe application approval claim

## Goal

Approved action records one admitted execution and receipt. Covers CAP-DB-REVIEW and REQ-DB-REVIEW.

## Context Digest

Actor: application reviewer and worker. Reachable entry: `reviewPayment workflow and ReviewTaskStore`. Contracts: CTR-DB-REVIEW. Current source is evidence only; approved spec digest is authority. Preserve existing uncommitted work and preceding ticket changes. No runtime work was performed by this plan.

## Implementation Approach

Replace consumed/consumeApproved with exact local ReviewTask/PaymentAction/ExecutionRecord/Receipt schemas and methods. Implement frozen creation expiry, bounded deterministic identities, CAS decision/claim/receipt state transitions, injected auth/revision/clock, idempotent reference executor. Bind invocation action before replay-skipped step; existing claim/receipt bypass new admission checks. Remove unsafe checkpoint casts and fixed 2030 deadline. Keep review CRUD/business identity outside Harness.
...
1. Read only declared read_scope relevant to this ticket. Check readiness/manifest and prerequisite lifecycle with CMD-SPECS/CMD-PLAN; record CMD-STATUS and baseline of existing ticket commands. New command files are tested after their test-first creation, not claimed to exist at baseline. **Phase Gate:** prerequisite TICKET-006 and matching digests must be proven before edits.
...
3. Extend the named tests before production behavior. All crash windows in review-execution.md, including effect-before-receipt and receipt-before-checkpoint; concurrent resumes; changed action after receipt and checkpoint; missing task after replay; mismatched receipt; unauthorized/stale/expired new admission; approved claim retry after expiry, revocation or revision callback outage; one logical effect/receipt/key; explicit fixed error reasons. Record intended failing assertions or exact already-existing proof for each acceptance row. Fixtures use fake providers/clocks and synthetic data only.
4. Implement in write_scope: Replace consumed/consumeApproved with exact local ReviewTask/PaymentAction/ExecutionRecord/Receipt schemas and methods. Implement frozen creation expiry, bounded deterministic identities, CAS decision/claim/receipt state transitions, injected auth/revision/clock, idempotent reference executor. Bind invocation action before replay-skipped step; existing claim/receipt bypass new admission checks. Remove unsafe checkpoint casts and fixed 2030 deadline. Keep review CRUD/business identity outside Harness. Keep public TSDoc current. **Phase Gate:** affected boundary tests and type checks must pass before dependent runtime/consumer behavior is marked implemented. If an external declaration/config mismatch is encountered, record blocked evidence; do not suppress or install.
...
6. Write evidence under `ai-harness/plans/decision-boundaries/evidence/TICKET-007.md` through the coordinator handoff. Implementation agent reports implemented/partial/blocked; coordinator owns index/lifecycle and manifest updates. Independent review checks specs plus this action plan before accepted. **Phase Gate:** next ticket stays blocked until this ticket is accepted and its dependency/index pointers are reconciled.

## Requirements Traceability

REQ-DB-REVIEW; CAP-DB-REVIEW. Success/failure/recovery paths and acceptance IDs are exact frontmatter links to 00-traceability.yaml; no deferrals.

## Contract Traceability

ai-harness/specs/37-decision-boundaries/03-contracts/decisions.md#CTR-DB-REVIEW, ai-harness/specs/37-decision-boundaries/03-flows/e2e-coverage.md#REQ-DB-REVIEW, ai-harness/specs/37-decision-boundaries/00-conventions.md#CONV-DB-STYLE. Approved decisions.md plus its explicitly linked provider-continuation/review-execution documents define values, error semantics and sequences; no local reinterpretation.

## Spec Drift Controls

Reject changed manifest digests, missing prerequisites, unregistered shapes, altered timeout/approval semantics, raw content in evidence or old API fallbacks. Review against the exact REQ-DB-REVIEW, CTR-DB-REVIEW and frontmatter acceptance rows before acceptance. Missing behavior goes to readiness review as a blocker, never an implementation guess.
...
All crash windows in review-execution.md, including effect-before-receipt and receipt-before-checkpoint; concurrent resumes; changed action after receipt and checkpoint; missing task after replay; mismatched receipt; unauthorized/stale/expired new admission; approved claim retry after expiry, revocation or revision callback outage; one logical effect/receipt/key; explicit fixed error reasons. Tests precede behavior; each success/failure/recovery acceptance row requires its exact assertion and command proof. Existing fixtures are extended; a new file is permitted only for a new module/checker or absent type-test seam. No live provider/credentials.

## Modularity And Reuse Plan

Existing wait/checkpoint/ExternalWaitOutcome and injected InMemoryHarnessStorage clock/failure seam; one application execution map, no generic core review subsystem. Placement follows 00-file-structure.md and 00-module-boundaries.yaml. Extract only helpers consumed by the named boundaries, not a new abstraction hierarchy. No unrelated domain logic.

## Representation Reuse Plan

Catalog: `ai-harness/specs/37-decision-boundaries/03-contracts/representation-catalog.yaml`. Shapes: review.action, review.descriptor, review.task, review.execution, review.receipt. Mappings: MAP-DB-ACTION, MAP-DB-CLAIM, MAP-DB-RECEIPT. Extend those owners only; no second envelope or renamed mirror. New-shape decision: none.

## Slice Strategy

Vertical slice runs from the named public/application entrypoint to its required effect, safe rejection and recovery outcome. Unblocks TICKET-008. No parallel writes: runtime builders, sessions and exports overlap across sequential tickets. Only read-only sidecar review is safe.
...
Approved action records one admitted execution and receipt. Unauthorized expired changed or conflicting action has no new effect. Every claim effect receipt checkpoint crash window resumes same execution identity. Every structured acceptance row must pass; no silent partial completion.

## Acceptance Test Matrix

| Acceptance | Required assertion | Verification |
| --- | --- | --- |
| ACC-REVIEW-SUCCESS | Approved action records one admitted execution and receipt | CMD-CORE-BUILD, CMD-REVIEW, CMD-REVIEW-TYPES, CMD-WAITS |
| ACC-REVIEW-FAILURE | Unauthorized expired changed or conflicting action has no new effect | CMD-CORE-BUILD, CMD-REVIEW, CMD-REVIEW-TYPES, CMD-WAITS |
| ACC-REVIEW-RECOVERY | Every claim effect receipt checkpoint crash window resumes same execution identity | CMD-CORE-BUILD, CMD-REVIEW, CMD-REVIEW-TYPES, CMD-WAITS |

## End-To-End Definition Coverage

`02-capabilities/capability-inventory.md#CAP-DB-REVIEW` and `03-flows/e2e-coverage.md#REQ-DB-REVIEW` specify actor, trigger, entrypoint, state, effects, authorization, errors, recovery, observability, owner and terminal outcomes. This ticket proves that chain through `ai-harness/examples/durable-human-review/src/payment-review.test.ts` and the matrix above. No additional UI/server surface is introduced.

## Operational Path Coverage

Cover all three PATH-DB-REVIEW paths. Safe reason/evidence and existing trace correlation, cancellation and idempotency follow the owning contract; no raw data logs. 04-nfr/requirements.md and 04-operations/runbook.md bind performance, integrity and release limits. External services, UI/a11y changes, dependency upgrades and production deployment are N/A, not simulated successes.
...
Run CMD-SPECS, CMD-PLAN, CMD-STATUS, CMD-CORE-BUILD, CMD-REVIEW, CMD-REVIEW-TYPES, CMD-WAITS. Commands are from workspace root and use installed tools. Their no-network/no-secrets contract applies to nested scripts. Local workspace artifacts only. Record expected red test proof before behavior, final pass counts, actual resolution paths and any failure. Do not download a tool or modify lockfiles to force a pass.
