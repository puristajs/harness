---
id: TICKET-004
title: Sound action tokens and schema-bound sensitive codecs
wave: 4
lifecycle: accepted
...
parallel_group: guardrail-authoring-sequential-4
depends_on:
  - TICKET-003
blocked_by: []
spec_refs:
  ...
  - ai-harness/specs/38-guardrail-authoring/03-flows/e2e-coverage.md#REQ-GA-ACTIONS
...
traceability:
  ...
  capability_ids:
    - CAP-GA-ACTIONS
  path_ids:
    - PATH-GA-ACTIONS-SUCCESS
    - PATH-GA-ACTIONS-FAILURE
    - PATH-GA-ACTIONS-RECOVERY
  acceptance_ids:
    - AC-GA-ACTIONS-SUCCESS
    - AC-GA-ACTIONS-FAILURE
    - AC-GA-ACTIONS-RECOVERY
generated_contracts:
...
acceptance:
  - id: ACC-ACTIONS-SUCCESS
    ...
    command_refs:
      ...
      - CMD-ADDON-TYPES
      - CMD-EXAMPLE
    expected_outcome: Literal action phase and schema output infer in inline and extracted callbacks; heterogeneous tokens compile with phase-constrained configuration; fixed and tool sensitive actions share algorithms.
    lifecycle: accepted
  - id: ACC-ACTIONS-FAILURE
    traceability_acceptance_ids:
      - AC-GA-ACTIONS-FAILURE
    requirement_refs:
      - ai-harness/specs/38-guardrail-authoring/03-flows/e2e-coverage.md#REQ-GA-ACTIONS
    ...
    command_refs:
      ...
      - CMD-ADDON-TYPES
      - CMD-EXAMPLE
    expected_outcome: Wrong phase target, false-transform, dynamic boolean, forged token, unsafe callback narrowing, missing codec schema and mistyped flow ID fail at their declared validation stage.
    lifecycle: accepted
  - id: ACC-ACTIONS-RECOVERY
    traceability_acceptance_ids:
      - AC-GA-ACTIONS-RECOVERY
    requirement_refs:
      - ai-harness/specs/38-guardrail-authoring/03-flows/e2e-coverage.md#REQ-GA-ACTIONS
    ...
    command_refs:
      ...
      - CMD-ADDON-TYPES
      - CMD-EXAMPLE
    expected_outcome: Existing decisions/evidence/timeouts/transforms remain single-owned; an unrelated tool is skipped before schema/codec/model work while a selected mismatch fails closed.
    lifecycle: accepted
---

## Goal

Sound action tokens and schema-bound sensitive codecs through the existing public entrypoint, including negative and recovery behavior.

## Context Digest

Read the listed contracts and analysis. This ticket owns REQ-GA-ACTIONS; previous tickets are dependencies, not permission to change their contracts. Source baseline is dirty. Voyage is excluded.

## Implementation Approach

- Introduce the opaque phase token and private prepared thunk; keep timers/evidence/telemetry and outcome validation solely in the existing coordinator.
- Replace permissive action/default generic declarations with schema-present/schema-omitted contextual overloads and exact transform branches.
- Constrain inline config flow IDs by token phase while retaining explicitly parsed dynamic configs; share the fixed sensitive-flow catalog.
- Implement tool selectors before protected-value validation; replace sensitive tool name options with singular sensitiveDataToolRail and required schema/selector.
- Convert modelCheckRail to a token with declared direct-alias metadata here; TICKET-005 owns projection and requirement aggregation.
- Update all addon example/test action declarations to the new constructor; do not expose partial typed APIs with legacy raw object acceptance.
...
REQ-GA-ACTIONS → CAP-GA-ACTIONS → SUCCESS/FAILURE/RECOVERY paths and AC IDs in frontmatter. No unowned acceptance remains.
...
1. Introduce the opaque phase token and private prepared thunk; keep timers/evidence/telemetry and outcome validation solely in the existing coordinator.
2. Replace permissive action/default generic declarations with schema-present/schema-omitted contextual overloads and exact transform branches.
3. Constrain inline config flow IDs by token phase while retaining explicitly parsed dynamic configs; share the fixed sensitive-flow catalog.
4. Implement tool selectors before protected-value validation; replace sensitive tool name options with singular sensitiveDataToolRail and required schema/selector.
5. Convert modelCheckRail to a token with declared direct-alias metadata here; TICKET-005 owns projection and requirement aggregation.
6. Update all addon example/test action declarations to the new constructor; do not expose partial typed APIs with legacy raw object acceptance.

## Acceptance

- Literal action phase and schema output infer in inline and extracted callbacks; heterogeneous tokens compile with phase-constrained configuration; fixed and tool sensitive actions share algorithms.
- Wrong phase target, false-transform, dynamic boolean, forged token, unsafe callback narrowing, missing codec schema and mistyped flow ID fail at their declared validation stage.
- Existing decisions/evidence/timeouts/transforms remain single-owned; an unrelated tool is skipped before schema/codec/model work while a selected mismatch fails closed.

## Acceptance Test Matrix

| Path | Required assertion |
| --- | --- |
| success | Literal action phase and schema output infer in inline and extracted callbacks; heterogeneous tokens compile with phase-constrained configuration; fixed and tool sensitive actions share algorithms. |
| failure | Wrong phase target, false-transform, dynamic boolean, forged token, unsafe callback narrowing, missing codec schema and mistyped flow ID fail at their declared validation stage. |
| recovery | Existing decisions/evidence/timeouts/transforms remain single-owned; an unrelated tool is skipped before schema/codec/model work while a selected mismatch fails closed. |
...
Write ai-harness/plans/guardrail-authoring/evidence/TICKET-004.md with exact results, acceptance-ID mapping and residual blockers. Mark implemented, not accepted. Independent reviewer controls acceptance; controller updates all four indexes and manifests together.
