---
id: TICKET-003
title: Docker-private indexed ownership and purge preparation
wave: 2
lifecycle: accepted
...
blocked_by: []
spec_refs: [ai-harness/specs/36-sandbox-ownership-and-administration/00-vision.md#REQ-SOWN-ADMIN, ai-harness/specs/36-sandbox-ownership-and-administration/00-vision.md#REQ-SOWN-BOUNDS, ai-harness/specs/36-sandbox-ownership-and-administration/00-vision.md#REQ-SOWN-SAFETY, ai-harness/specs/36-sandbox-ownership-and-administration/00-vision.md#REQ-SOWN-DELIVERY, ai-harness/specs/36-sandbox-ownership-and-administration/03-contracts/api.md#CTR-SOWN-OWNER, ai-harness/specs/36-sandbox-ownership-and-administration/03-contracts/api.md#CTR-SOWN-ADMIN, ai-harness/specs/36-sandbox-ownership-and-administration/03-contracts/api.md#CTR-SOWN-OPTIONS, ai-harness/specs/36-sandbox-ownership-and-administration/03-contracts/api.md#CTR-SOWN-ERRORS, ai-harness/specs/36-sandbox-ownership-and-administration/04-delivery.md#DEC-SOWN-DELIVERY]
write_scope: [ai-harness/packages/harness-sandbox-docker/src/ownership.ts, ai-harness/packages/harness-sandbox-docker/src/ownership.test.ts, ai-harness/packages/harness-sandbox-docker/src/administration.ts, ai-harness/packages/harness-sandbox-docker/src/administration.test.ts]
...
contract_readiness:
  ...
  missing_contracts: []
traceability:
  requirement_ids: [REQ-SOWN-ADMIN, REQ-SOWN-BOUNDS, REQ-SOWN-SAFETY, REQ-SOWN-DELIVERY]
  capability_ids: [CAP-SOWN-ADMIN, CAP-SOWN-BOUNDS, CAP-SOWN-SAFETY, CAP-SOWN-DELIVERY]
  path_ids: [PATH-SOWN-ADMIN, PATH-SOWN-BOUNDS, PATH-SOWN-SAFETY, PATH-SOWN-DELIVERY]
  acceptance_ids: [ACC-SOWN-ADMIN, ACC-SOWN-BOUNDS, ACC-SOWN-SAFETY, ACC-SOWN-DELIVERY]
generated_contracts:
  status: ready
...
action_steps:
  - id: STEP-PREFLIGHT
    ...
    acceptance_refs: [AC-TICKET-003-ADMIN, AC-TICKET-003-BOUNDS, AC-TICKET-003-SAFETY, AC-TICKET-003-DELIVERY]
  ...
  - id: STEP-CONTRACT
    kind: contract
    files: [ai-harness/specs/36-sandbox-ownership-and-administration/03-contracts/api.md, ai-harness/packages/harness-sandbox-docker/src/ownership.ts]
  ...
  - id: STEP-TEST
    kind: test
    files: [ai-harness/packages/harness-sandbox-docker/src/ownership.test.ts, ai-harness/packages/harness-sandbox-docker/src/administration.test.ts]
  ...
  - id: STEP-IMPLEMENT
    kind: implement
    files: [ai-harness/packages/harness-sandbox-docker/src/ownership.ts, ai-harness/packages/harness-sandbox-docker/src/ownership.test.ts, ai-harness/packages/harness-sandbox-docker/src/administration.ts, ai-harness/packages/harness-sandbox-docker/src/administration.test.ts]
    command_refs: [CMD-DOCKER_TEST, CMD-DOCKER_TYPES]
    acceptance_refs: [AC-TICKET-003-ADMIN, AC-TICKET-003-BOUNDS, AC-TICKET-003-SAFETY, AC-TICKET-003-DELIVERY]
  ...
  - id: STEP-VERIFY
    kind: verify
    files: [ai-harness/packages/harness-sandbox-docker/src/ownership.test.ts, ai-harness/packages/harness-sandbox-docker/src/administration.test.ts]
    command_refs: [CMD-SPEC, CMD-PLAN, CMD-DOCKER_TEST, CMD-DOCKER_TYPES]
    acceptance_refs: [AC-TICKET-003-ADMIN, AC-TICKET-003-BOUNDS, AC-TICKET-003-SAFETY, AC-TICKET-003-DELIVERY]
...
# TICKET-003 — Docker-private indexed ownership and purge preparation
...
1. Consume public Harness DTO schemas/types only; keep opaque engine IDs and indexed owner fields in the Docker-private journal.

2. Prepare owner registration, actor revocation, indexed list/purge, tombstone reservations and cleanup retries with exact existing-label ownership validation.

3. Script container stop/remove and volume deletion outcomes; pending creates and missing/denied/timeout responses cannot claim completed deletion.

4. Do not change image acquisition, networking, mounts, exec fallback or capability claims. No real engine call or image pull belongs to this foundation.
...
Container/volume deletion failures retain refs; stop-before-delete and principal-specific attach revocation; unknown ownership never adopted; unsupported hard volume quota rejected; no provider refs in public output. Run the listed test files before the behavior change, then after; record exact red and green results. Include cancellation/denial/retry/state-loss assertions and content-free error projection when those paths are in scope.

## Modularity And Reuse Plan

Reuse the modules and public exports in ai-harness/specs/36-sandbox-ownership-and-administration/00-reuse-inventory.yaml and 04-delivery.md. New helper files stay inside the owned domain folders. Extend existing catalog/error/identity/SQLite/engine mechanisms; no public lifecycle database port or generic resource framework. Docker must not import private Harness catalog code.
...
Approved horizontal foundation exception: only contract/private primitives, with direct tests and no parallel runtime contract. Atomic base-port cutover consumes these primitives in wave 3.
...
| --- | --- | --- |
| AC-TICKET-003-ADMIN | ACC-SOWN-ADMIN in 04-verification.md | ai-harness/packages/harness-sandbox-docker/src/ownership.test.ts, ai-harness/packages/harness-sandbox-docker/src/administration.test.ts |
| AC-TICKET-003-BOUNDS | ACC-SOWN-BOUNDS in 04-verification.md | ai-harness/packages/harness-sandbox-docker/src/ownership.test.ts, ai-harness/packages/harness-sandbox-docker/src/administration.test.ts |
| AC-TICKET-003-SAFETY | ACC-SOWN-SAFETY in 04-verification.md | ai-harness/packages/harness-sandbox-docker/src/ownership.test.ts, ai-harness/packages/harness-sandbox-docker/src/administration.test.ts |
| AC-TICKET-003-DELIVERY | ACC-SOWN-DELIVERY in 04-verification.md | ai-harness/packages/harness-sandbox-docker/src/ownership.test.ts, ai-harness/packages/harness-sandbox-docker/src/administration.test.ts |

Exact expected cases: Container/volume deletion failures retain refs; stop-before-delete and principal-specific attach revocation; unknown ownership never adopted; unsupported hard volume quota rejected; no provider refs in public output.

## End-To-End Definition Coverage

CAP-SOWN-ADMIN / PATH-SOWN-ADMIN; CAP-SOWN-BOUNDS / PATH-SOWN-BOUNDS; CAP-SOWN-SAFETY / PATH-SOWN-SAFETY; CAP-SOWN-DELIVERY / PATH-SOWN-DELIVERY map to ai-harness/specs/36-sandbox-ownership-and-administration/02-capabilities/capability-inventory.md and 03-flows/e2e-coverage.md. Those rows fix actor, reachable entrypoint, data/state, side effects, permissions, errors/recovery, owner/final state and observability. This ticket covers the implementation approach above; no frontend/client screen or new transport is introduced.

## Operational Path Coverage

Include the matching failure/recovery cases in 04-verification.md: Container/volume deletion failures retain refs; stop-before-delete and principal-specific attach revocation; unknown ownership never adopted; unsupported hard volume quota rejected; no provider refs in public output. Operator/provider actions outside default commands stay blocked until separately authorized. Cleanup remains retryable and never reports partial work as completed.
