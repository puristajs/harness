...
- A session id scopes state; it is not a distributed conversation scheduler. The application chooses whether same-session turns serialize, reject while busy, or use independent sessions. Atomic state writes and timestamps do not make parallel model calls causally ordered.
