...
| CAP-SBX-DOCKER | Execute in a local Docker container using one optional addon, including OrbStack | standalone application or PURISTA composition root | `dockerSandbox(options)` through ordinary `Sandbox` | retained files and reattachment on one engine, explicit state loss, scoped cleanup; no new checkpoint system |
