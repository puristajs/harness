...
- Prefer schema-first generics. Infer callback/context/session types from the immutable definition object; do not mirror user-authored shapes in interfaces.
...
- Preserve literal definition ids and root-only `$infer.agents`/`$infer.workflows`
  members. Preserve exact dependency types privately without publishing the
  recursive closure. Do not widen to `string`, `unknown`, `any`,
  `Record<string, ...>`, or `JsonValue` when an exact inferred type exists.
