...
| --- | --- | --- | --- | --- |
| CAP-SS-TYPES | Vendor-neutral schema declarations with exact nested inference | harness core | package exports and definitions | CTR-SS-SCHEMA, CTR-SS-DEFINITIONS |
...
| CAP-SS-PROJECTION | Definition-time model JSON Schema projection and cache | harness core | definition and graph compilation | CTR-SS-PROJECTION, CTR-SS-ERRORS |
...
- **AC-SS-PROJECTION-SUCCESS:** Each tool input/structured-agent output converter runs once during definition/graph compilation, receives Draft 2020-12 input mode, and yields a recursively frozen `JsonValue` reused across runs/retries.
- **AC-SS-PROJECTION-FAILURE:** Missing, throwing or invalid projection fails definition/graph compilation with the exact closed error reason/boundary/id/target metadata and no vendor content.
- **AC-SS-PROJECTION-RECOVERY:** Recreating the definition with a valid `ModelSchema` succeeds; no partial graph from the failed compilation is runnable.
