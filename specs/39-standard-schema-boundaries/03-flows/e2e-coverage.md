...
| --- | --- | --- |
| PATH-SS-TYPES-SUCCESS | Define vendor schemas → direct factory retains schema generics → IDE/type fixtures expose exact nested input/output | equality and positive declaration fixtures for three vendors |
| PATH-SS-TYPES-FAILURE | Define invalid/model-incomplete/non-JSON schema or wrong invocation → compiler evaluates constraints → definition fixture fails | negative `@ts-expect-error` fixtures, including two incompatible aliases |
...
| PATH-SS-VALIDATION-RECOVERY | Invoke the same Harness instance with valid data after failure → no failure state retained → run completes | failure-then-success test per runtime family |
| PATH-SS-PROJECTION-SUCCESS | Define with `ModelSchema` → input projection once, JSON check and freeze → cached schema serves all runs/retries | converter spy count, target assertion, recursive freeze and identity tests |
| PATH-SS-PROJECTION-FAILURE | Projection missing/throws/returns invalid value → compilation emits closed `HarnessConfigError` → no runnable graph | three reason-specific tests and privacy snapshot |
| PATH-SS-PROJECTION-RECOVERY | Correct schema and recreate definition → compilation is complete → runs consume new cached schema | failed then successful definition test |
...
| --- | --- | --- | --- |
| agent | before prompt/loop/model call | every structured model candidate before completion | output schema at definition compilation |
| TypeScript tool | model arguments before authorization/handler | handler return before model payload/persistence | input schema at definition compilation |
...
Cancellation, telemetry and logging follow the existing run lifecycle. Async
validation introduces no detached promises. Recovery means a new invocation or
definition with corrected input/schema, never silent coercion or provider
keyword removal.
